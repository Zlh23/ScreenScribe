"""
Train AffineColorContrastNet from scratch (same data & loss as ColorContrastNet).
fg = mul * bg_oklch + add; mul/add from two MLPs(weights_4, want_lchc_4).
Saves to checkpoints/AffineColorContrastNet/.
"""
...
class AffineColorContrastLightningModule(pl.LightningModule):
    def __init__(
        self,
        ...
        self.net = AffineColorContrastNet(hidden=hidden)
...
class SaveAffineCheckpointCallback(ModelCheckpoint):
    """Save AffineColorContrastNet checkpoint with model_type, hidden (shared trunk)."""
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        if epoch % self.save_every == 0:
            ...
            torch.save({
                ...
                "model_type": "AffineColorContrastNet",
                "hidden": self.hidden,
            }, pth)
            print(f"  saved {pth}")
...
def main() -> None:
    p = argparse.ArgumentParser(description="Train AffineColorContrastNet (fg = mul*bg + add)")
    ...
    p.add_argument("--hidden", type=int, nargs="+", default=[128, 128, 64], help="shared trunk hidden layers")
    args = p.parse_args()
    ...
    hidden = args.hidden
    model = AffineColorContrastLightningModule(
        hidden=hidden,
        lr=args.lr,
    ...
    model_dir = Path(args.save_dir) / AffineColorContrastNet.__name__
    ...
    save_callback = SaveAffineCheckpointCallback(
        model_dir=model_dir,
        save_every=args.save_every,
        hidden=hidden,
    )
    log_dir = Path(args.log_dir)
    logger = TensorBoardLogger(str(log_dir), name="affine_color_contrast")
    csv_logger = CSVLogger(str(log_dir), name="affine_color_contrast")
    ...
    print(f"AffineColorContrastNet: shared trunk hidden={hidden}")
    print(f"Device: {args.device} (accelerator={accelerator}, devices={devices})")
    trainer = pl.Trainer(
